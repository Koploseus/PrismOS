 ...
 * Enables agents to execute transactions on behalf of users with scoped permissions.
 * 
 * KEY INSIGHT: Session keys must be installed on-chain via initConfig!
 * Just creating a kernel account with `regular: permissionValidator` does NOT
 * persist the permissions on-chain. We must:
 * 1. Use `initConfig: toInitConfig(permissionPlugin)` 
 * 2. Send a UserOp to deploy the wallet + install the permissions
 */
...
const CHAIN_ID: ChainId = (parseInt(process.env.NEXT_PUBLIC_CHAIN_ID || String(DEFAULT_CHAIN_ID)) as ChainId);
...
function getConfig(projectId: string, chainId: ChainId = CHAIN_ID) {
  const useLocalFork = typeof window !== 'undefined' &&
    process.env.NEXT_PUBLIC_USE_LOCAL_FORK === 'true';
...
function createAgentPolicy() {
  // DEMO: sudo policy for hackathon - allows any transaction
  return toSudoPolicy({});

  // PRODUCTION: restrictive call policy
  // return toCallPolicy({
...
export async function createSessionKey(
  ...
  try {
    ...
    // Step 6: Create kernel account WITH initConfig to install permissions
    // THIS IS THE KEY DIFFERENCE - using initConfig ensures permissions are
    // installed on-chain when the wallet is deployed. Without initConfig,
    // regular: permissionPlugin does NOT persist permissions on-chain.
    ...
    // Send UserOp to deploy wallet + install permissions (user signs this)
    // Dummy call to zeroAddress - the real work is in initCode during wallet creation
